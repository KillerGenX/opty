...
export async function POST(req: Request) {
  try {
    ...
    let { opportunityId, docType, referenceImage, additionalContext, previousDraft } = body

    // Strip massive base64 images from previousDraft to prevent exceeding 1M token limit!
    if (previousDraft && typeof previousDraft === 'string') {
      previousDraft = previousDraft.replace(/data:image\/[^;]+;base64,[^"]+/g, '[BASE64_IMAGE_REMOVED]')
    }
    ...
    if (docType === 'concept_art') {
      try {
        ...
        // Pass the uploaded reference image directly to Nano Banana Pro for multimodal image generation!
        const base64Img = await generateConceptImage(parsed.image_prompt, referenceImage)
        
        // Wrap the Image in HTML
        htmlContent = `<div class="my-8 flex flex-col items-center justify-center w-full bg-white p-6 rounded-xl border border-slate-200 shadow-sm concept-art-container">\n<h3 class="font-semibold text-lg text-slate-800 mb-4">Solution Infographic Poster</h3>\n<img src="data:image/jpeg;base64,${base64Img}" alt="Solution Infographic Poster" class="w-full max-w-4xl rounded-lg shadow-md" />\n<p class="text-[10px] text-slate-400 mt-6 italic text-center w-full border-t border-slate-100 pt-3"><span class="font-medium text-emerald-600">Visuals generated by Gemini 3 Pro Image (Nano Banana Pro)</span>. High-fidelity typographic rendering enabled.</p>\n</div>`
      } catch (err: any) {
